 ...
 */
// ❌ Option a — repetitive
function getNumberFirst(arr) {
    return arr[0];
}
function getStringFirst(arr) {
    return arr[0];
}
...
const numberBox = { value: 42, label: 'age' };
const stringBox = { value: 'Zaffar', label: 'name' };
...
const coord = { first: 99, second: 98 };
...
class KeyValueStore {
    constructor() {
        this.store = {}; // Record<K, V> is a built-in TypeScript utility type.
    }
    set(key, value) {
...
class EventEmitter {
    ...
    emit(event, data) {
        var _a;
        (_a = this.listeners[event]) === null || _a === void 0 ? void 0 : _a.forEach((fn) => fn(data));
    }
}
